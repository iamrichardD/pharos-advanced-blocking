...
- `pab map --ip <ip> --group <group>` — Add or update a client IP mapping
- `pab deploy --dry-run` — Preview changes before applying
- `pab deploy -f` — Apply changes to your Technitium instances in CI/CD
...
### 4. Use Tab Completion for Command Names
...
│  Tip: Press Tab in search to navigate through filtered results   │
│                                                                  │
│                                                                  │
...
- **Exploring your configuration**: Browse all clients, groups, and current assignments
- **Debugging DNS issues**: Search for a specific client IP, see what group it's assigned to, check blocklists
- **Making one-off changes**: Quickly view a group's settings or search results
- **Learning the system**: New to Pharos? The TUI guides you interactively with real-time search and `/help` commands

**Launch with**: `pab` (no arguments)

### Command Line (CLI) — Use When...

- **Automating bulk operations**: Map 100 client IPs at once with `pab map` commands
- **Setting up CI/CD pipelines**: Deploy changes automatically on schedule or triggered by events
- **Integrating with other tools**: Call `pab map`, `pab unmap`, or `pab deploy` from your scripts
- **Production deployments**: Use `pab deploy --dry-run` to preview changes before applying with `-f`
- **Non-interactive environments**: Running in containers or cron jobs where a terminal isn't available

**Commands**: `pab map`, `pab unmap`, `pab deploy`, `pab --help`
...
- **IP Filtering**: Type `192.0` and results filter to show only IPs containing "192.0"
- **Group Filtering**: Type `kid` and results filter to show only groups containing "kid" (case-insensitive)
- **Tab Navigation**: Press Tab to cycle through matching IPs or groups
- **Enter to Confirm**: Press Enter to apply the selected filter
...
- **Full IP autocomplete**: Typing `192.0[Tab]` will NOT auto-complete to a full IP like `192.0.2.50`—it filters and navigates through matches
- **Full group name autocomplete**: Typing `kid[Tab]` will NOT auto-complete to "Kids"—it filters and navigates through matches
- **Auto-completion of group names is planned for a future release**
...
- **Group name Tab completion in search**: Typing `kid[Tab]` will NOT auto-complete to "Kids"—it filters and navigates search results instead
- **Subcommand Tab completion**: Typing `/view gr[Tab]` will NOT complete to "/view groups"—you must type "groups" manually
- **These features are planned for a future release**

### How It's Different From Search

- **Command Tab completion**: TRUE autocomplete. `/v[Tab]` → `/view ` (fills in the space automatically)
- **Search Tab**: Navigate through filtered results. `kid[Tab]` → Cycles through matching groups but does NOT complete the name
